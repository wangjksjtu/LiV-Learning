...
class DVR_FMAP_Provider:
    ...
    def load_one_batch(self, batch_size, Type='train', shape1=[66, 200], shape2=[300, 502]):
        x_out1 = []
        x_out2 = []
        y_out = []
        if Type == 'train':
            ...
            for i in range(0, batch_size):
                ...
                x_out2.append(scipy.misc.imresize(scipy.misc.imread(
                self.val_xs2[index]), shape) / 255.0)
                y_out.append(self.val_ys[index])
                self.val_pointer += batch_size
        return np.stack(x_out1), np.stack(x_out2), np.stack(y_out)

    def load_val_all(self, batch_size, shape1 = [66, 200], shape2 = [300, 502]):
        x_out1 = []
        x_out2 = []
        y_out = []
        index = 0
